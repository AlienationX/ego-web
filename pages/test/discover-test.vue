<template>
    <view class="layout" :class="settingsStore.isDark ? 'theme-dark' : 'theme-light'">
        ...
        <!-- 返回测试主页浮动按钮 -->
        <view class="test-back-btn" :style="{ top: `${statusBarHeight + 6}px` }" @click="goBack">
            <uni-icons type="left" size="20" :color="settingsStore.isDark ? '#f3f4f6' : '#1f2937'"></uni-icons>
        </view>
...
const goBack = () => {
    ...
    if (pages.length > 1) {
        ...
        uni.reLaunch({ url: '/pages/test/test' });
    }
};
