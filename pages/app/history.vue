    ...
    <view class="layout" :class="isDark ? 'theme-dark' : 'theme-light'">
        <view class="nav-container">
            ...
            <view class="header" :style="{ height: `${titleBarHeight}px`, paddingRight: titlebarPaddingRight }">
                ...
                <!-- #ifdef MP-WEIXIN -->
                <view class="clear-all clear-all--icon" v-if="libraryStore.recentViewed.length" @click="clearAllHistory">
                    <mdi-icon path="/static/icons/delete-empty.svg" size="18px" :color="isDark ? '#ff6b6b' : '#ef4444'"></mdi-icon>
                </view>
                <view v-else class="header-placeholder header-placeholder--icon"></view>
                <!-- #endif -->
                
                <!-- #ifndef MP-WEIXIN -->
                <view class="clear-all" v-if="libraryStore.recentViewed.length" @click="clearAllHistory">
                    <mdi-icon path="/static/icons/delete-empty.svg" size="18px" :color="isDark ? '#ff6b6b' : '#ef4444'"></mdi-icon>
                    <text class="clear-all__text" :style="{ color: isDark ? '#ff6b6b' : '#ef4444' }">{{ t('historyPage.clearAll') }}</text>
                </view>
        ...
        <scroll-view scroll-y class="content" @scrolltolower="loadMore">
            <view v-if="libraryStore.recentViewed.length === 0" class="empty-state">
                <image class="empty-icon" src="/static/logo.svg" mode="aspectFit"></image>
                ...
                <view v-for="day in visibleDays" :key="day.dateStr" class="day-section">
                    <view class="day-header">
                        ...
                        <view class="day-clear" @click="clearDayHistory(day.dateStr)">
                            <mdi-icon path="/static/icons/delete-empty.svg" size="18px" :color="isDark ? '#6b7280' : '#9ca3af'"></mdi-icon>
                    ...
                    <view class="day-grid">
                        <view 
                            ...
                            :key="item.id" 
                            class="card-item"
                            @click="goPreview(item.id, day.items)"
                        >
...
const clearDayHistory = (dateStr) => {
    showDeleteDialog({
        ...
        description: t('historyPage.clearDayConfirm', { date: getDayLabel(dateStr) }) || `确定要清除 ${getDayLabel(dateStr)} 的浏览记录吗？`,
        onConfirm: () => {
