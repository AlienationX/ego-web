<template>
    <view class="layout" :class="settingsStore.isDark ? 'theme-dark' : 'theme-light'">
        ...
        <view class="timeline-wrap" :style="{ paddingTop: timelineWrapPaddingTop, paddingBottom: timelineWrapPaddingBottom }">
                <view class="topbar">
...
const timelineWrapPaddingTop = computed(() => `${statusBarHeight.value + 10}px`);
const timelineWrapPaddingBottom = '24px';
...
onLoad((options) => {
    if (options?.unreadCount) {
        unreadCount.value = parseInt(options.unreadCount, 10);
    }
});
...
const monthGroups = computed(() => {
    ...
    latestList.value.forEach((item, index) => {
        ...
        if (unreadCount.value > 0 && index === unreadCount.value - 1) {
