...
watch(
    ...
    (newVal, oldVal = []) => {
        ...
        if (newVal.length <= oldVal.length) {
            ...
            const addVal = newVal
                .filter((item) => !listIds.has(item.id))
                .map((item) => reactive(JSON.parse(JSON.stringify(item))));
...
const updateLayout = () => {
    ...
    waterfallLayout.columnWidth =
        (containerWidth - (waterfallLayout.columnCount + 1) * gap.value) / waterfallLayout.columnCount;
